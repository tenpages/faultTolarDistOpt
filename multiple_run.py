...
# 						'--err-mode', fault_type,
# 						'--lr', '0.01',
# 						'--train-dir', 'output/models/approx/CIFAR10/10-'+str(i)+'/'+model_name+'/'+fault_name+'/',
# 						'--seed', '0',
# 						'--accumulative', 'False',
# 						'--worker-fail', str(i),
# 						'--fault-thrshld', str(i),
...
# 				results = subprocess.run(args, capture_output=True)
# 				if results.returncode==0 and results.stdout != None:
# 					with open('logs-approx-CIFAR10-ResNet18-' + batch_size + '-' + model_name + '-' + fault_name + '-10-' + str(i),'w') as f:
# 						f.write(results.stdout.decode())
# 					print("finished")
# 					print("========================")
# 				else:
...
# 		'--err-mode', fault_type,
# 		'--lr', '0.01',
# 		'--train-dir', 'output/models/approx/CIFAR10/10-3/normal/',
# 		'--seed', '0',
# 		'--accumulative', 'False',
# 		'--worker-fail', '3',
# 		'--fault-thrshld', '3',
...
# results = subprocess.run(args, capture_output=True)
# if results.returncode==0 and results.stdout != None:
# 	with open('logs-approx-CIFAR10-ResNet18-' + batch_size + '-normal-10-3','w') as f:
# 		f.write(results.stdout.decode())
# 	print("finished")
# 	print("========================")
# else:
...
# 		for fault_type, fault_name in zip(fault_types, fault_names):
# 			for model_name, model in zip(model_names, models):
# 				args = 'python distributed_eval.py --model-dir output/models/approx/CIFAR10/10-'+str(i)+'/'+model_name+'/'+fault_name \
# 						+'/ --dataset CIFAR10 --network ResNet18 --eval-freq 1 > "results-approx-CIFAR10-ResNet18-'+batch_size+'-'+model_name+'-'+fault_name+'-10-'+str(i)+'" 2>&1 &'
# 				print("Now evaluating "+fault_name+" using "+model_name+" using command:")
# 				print(args)
# 				results = subprocess.run(args, shell=True)
#
...
# fault_name = 'revgrad2'
# model_name = 'normal'
# args = 'python distributed_eval.py --model-dir output/models/approx/CIFAR10/10-3/normal/' \
# 		+' --dataset CIFAR10 --network ResNet18 --eval-freq 1 > "results-approx-CIFAR10-ResNet18-'+batch_size+'-normal-10-3" 2>&1 &'
# print("Now evaluating "+fault_name+" using "+model_name+" using command:")
# print(args)
# results = subprocess.run(args, shell=True)
