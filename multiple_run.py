...
# 							'--err-mode', fault_type,
# 							'--lr', '0.01',
# 							'--train-dir', 'output/models/async/async-fmnist/run' + str(k) + '/' + fault_name + '-' + str(i) + '/',
# 							'--seed', str(k),
# 							'--accumulative', 'False',
# 							'--worker-fail', str(i),
# 							'--fault-thrshld', str(i),
			...
			for fault_type, fault_name in zip(fault_types, fault_names):
				for model_name, model in zip(model_names, models):
					args = ['mpirun', '-n', '21',
							...
							'--err-mode', fault_type,
							'--lr', '0.01',
							'--train-dir', 'output/models/async/async-mnist/run' + str(k) + '/' + fault_name + '-' + str(i) + '/',
							'--seed', str(k),
							'--accumulative', 'False',
							'--worker-fail', str(i),
							'--fault-thrshld', str(i),
...
# 			for fault_type, fault_name in zip(fault_types, fault_names):
# 				for model_name, model in zip(model_names, models):
# 					args = 'python distributed_eval.py --model-dir output/models/async/async-fmnist/run'+str(k)+'/'+fault_name+'-'+str(i)\
...
for k in [1,2,3,4]:
	for batch_size in batch_sizes:
		for i in nums_faults:
			for fault_type, fault_name in zip(fault_types, fault_names):
				for model_name, model in zip(model_names, models):
					args = 'python distributed_eval.py --model-dir output/models/async/async-mnist/run'+str(k)+'/'+fault_name+'-'+str(i)\
