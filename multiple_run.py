		...
		for fault_type, fault_name in zip(fault_types, fault_names):
			for model_name, model in zip(model_names, models):
				args = ['mpirun', '-n', '21',
						...
						'--err-mode', fault_type,
						'--lr', '0.01',
						'--train-dir', 'output/models/async/sync/',
						'--accumulative', 'False',
						'--worker-fail', '0',
						'--fault-thrshld', '0',
						'--data-distribution', 'same',
				...
				results = subprocess.run(args, capture_output=True)
				if results.returncode==0 and results.stdout != None:
					with open('logs-async-MNIST-LeNet-' + batch_size + '-sync-20-' + str(i),'w') as f:
						f.write(results.stdout.decode())
					print("finished")
					print("========================")
				else:
...
for batch_size in batch_sizes:
	for i in nums_faults:
		for fault_type, fault_name in zip(fault_types, fault_names):
			for model_name, model in zip(model_names, models):
				args = 'python distributed_eval.py --model-dir output/models/async/sync'\
						+'/ --dataset MNIST --network LeNet --eval-freq 1 > "results-async-MNIST-LeNet-'+batch_size+'-sync-20-'+str(i)+'" 2>&1 &'
				print("Now evaluating "+fault_name+" using "+model_name+" using command:")
				print(args)
				results = subprocess.run(args, shell=True)
